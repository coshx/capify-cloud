class Images
  ...
  def initialize(compute_connection, role, stage, app_name)
  ...
  def create(prototype_instance)
    puts "  creating new image - this will take a few minutes" unless Fog.mocking?
    time =  Time.now.utc.iso8601.gsub(':','.')
    image_name = "#{@role}.#{@stage}.#{@app_name}"
    ...
    Fog.wait_for do
      begin
      STDOUT.write "\r  #{progress_output}" unless Fog.mocking?
    ...
    if image.state == 'failed' && !Fog.mocking?
      ...
      puts 'creating tags' unless Fog.mocking?
      ...
      Fog.wait_for do
        image = @compute_connection.images.get(image.id)
        !image.tags.empty?
      end
